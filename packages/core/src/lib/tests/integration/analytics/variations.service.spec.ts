...
} from '../../../types/analytics/variations/index.js';
import {
  getWpUrl,
  getAdminUser,
  getAdminAppPassword,
...
describe('Analytics Variations — integration', () => {
  it('returns variation stats with valid shape', async () => {
    const { data, error } = await sdk.analytics.variations.getStats({
    ...
    const parsed = AnalyticsVariationsStatsResponseSchema.safeParse(data);
    expect(
      parsed.success,
  ...
  it('returns a list of variation rows with valid shape', async () => {
    const result = await sdk.analytics.variations.list({ per_page: 5 });
    ...
    for (const item of result.data ?? []) {
      const parsed = AnalyticsVariationSchema.safeParse(item);
      expect(
        parsed.success,
  ...
  it('returns pagination metadata in list response', async () => {
    const result = await sdk.analytics.variations.list({ per_page: 2 });
