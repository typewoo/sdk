...
} from '../../../types/analytics/products/index.js';
import {
  getWpUrl,
  getAdminUser,
  getAdminAppPassword,
...
describe('Analytics Products — integration', () => {
  it('returns product stats with valid shape', async () => {
    ...
    const parsed = AnalyticsProductsStatsResponseSchema.safeParse(data);
    expect(
      parsed.success,
  ...
  it('returns a list of product rows with valid shape', async () => {
    const result = await sdk.analytics.products.list({
      after: '2025-01-01T00:00:00',
      before: '2026-12-31T23:59:59',
      per_page: 5,
    });
    ...
    for (const item of result.data ?? []) {
      const parsed = AnalyticsProductSchema.safeParse(item);
      expect(
        parsed.success,
  ...
  it('returns pagination metadata in list response', async () => {
    const result = await sdk.analytics.products.list({ per_page: 2 });
