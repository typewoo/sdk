...
} from '../../../types/analytics/taxes/index.js';
import {
  getWpUrl,
  getAdminUser,
  getAdminAppPassword,
...
describe('Analytics Taxes — integration', () => {
  it('returns tax stats with valid shape', async () => {
    const { data, error } = await sdk.analytics.taxes.getStats({
    ...
    const parsed = AnalyticsTaxesStatsResponseSchema.safeParse(data);
    expect(
      parsed.success,
  ...
  it('returns a list of tax rows with valid shape', async () => {
    const result = await sdk.analytics.taxes.list({ per_page: 5 });
    ...
    for (const item of result.data ?? []) {
      const parsed = AnalyticsTaxSchema.safeParse(item);
      expect(
        parsed.success,
  ...
  it('returns pagination metadata in list response', async () => {
    const result = await sdk.analytics.taxes.list({ per_page: 2 });
