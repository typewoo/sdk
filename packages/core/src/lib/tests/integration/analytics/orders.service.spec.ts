...
} from '../../../types/analytics/orders/index.js';
import {
  getWpUrl,
  getAdminUser,
  getAdminAppPassword,
...
describe('Analytics Orders — integration', () => {
  it('returns order stats with valid shape', async () => {
    const { data, error } = await sdk.analytics.orders.getStats({
    ...
    const parsed = AnalyticsOrdersStatsResponseSchema.safeParse(data);
    expect(
      parsed.success,
  ...
  it('returns a list of order rows with valid shape', async () => {
    const result = await sdk.analytics.orders.list({
      after: '2025-01-01T00:00:00',
      before: '2026-12-31T23:59:59',
      per_page: 5,
    });
    ...
    for (const item of result.data ?? []) {
      const parsed = AnalyticsOrderSchema.safeParse(item);
      expect(
        parsed.success,
  ...
  it('returns pagination metadata in list response', async () => {
    const result = await sdk.analytics.orders.list({ per_page: 2 });
