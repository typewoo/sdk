...
import { doRequest as doRequestWithConfig } from '../../../http/index.js';
import type { RequestOptions } from '../../../types/request.js';
import type { ResolvedSdkConfig } from '../../../configs/sdk.config.js';

/**
 * Global callbacks come from the SDK instance's config, which is passed to
 * doRequest explicitly. Each test sets the config it wants with
 * setSdkConfig(), and doRequest() below passes it along.
 */
let sdkConfig: ResolvedSdkConfig | undefined;
const setSdkConfig = (config: ResolvedSdkConfig | null) => {
  sdkConfig = config ?? undefined;
};
const doRequest = (
  instance: AxiosInstance,
  url: string,
  options: RequestOptions
) => doRequestWithConfig(instance, url, options, sdkConfig);
