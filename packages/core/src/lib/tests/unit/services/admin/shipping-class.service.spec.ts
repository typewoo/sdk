...
import { AdminShippingClassService } from '../../../../services/admin/shipping-class.service.js';
...
describe('AdminShippingClassService', () => {
  ...
  describe('get()', () => {
    it('calls GET /wc/v3/products/shipping_classes/{id}', async () => {
      ...
      expect(doGetMock.mock.calls[0][0]).toContain(
        '/wp-json/wc/v3/products/shipping_classes/1'
      );
  ...
  describe('create()', () => {
    it('POSTs to /wc/v3/products/shipping_classes', async () => {
      ...
      const result = await svc.create({ name: 'Fragile' });
