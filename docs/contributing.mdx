...
New services should extend `BaseService` and make requests through
`this.http`, which is bound to the owning SDK instance:
...
export class NewService extends BaseService {
  ...
  async list(options?: RequestOptions): Promise<ApiResult<ResponseType[]>> {
    ...
    const { data, error, headers } = await this.http.get<ResponseType[]>(
      url,
      options
    );
...
const { data, error } = await this.http.get(url, options);
...
```typescript
// Correct
const { data, error } = await this.http.get<T>(url);
return { data, error };
...
export class NewService extends BaseService {
  ...
  async list(options?: RequestOptions): Promise<ApiResult<NewResponse[]>> {
    const url = `/${this.endpoint}`;
    const { data, error } = await this.http.get<NewResponse[]>(url, options);
    return { data, error };
  }
}
```
