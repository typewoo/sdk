...
| --------- | ------------------------------ | -------------------------- |
| `loop()`  | Iterate all pages, return last | All paginated list methods |

---

## Loop Extension

The `loop()` extension iterates through all pages of a paginated endpoint and returns only the **last page's data**. It's available on any `list()` method that returns paginated data. Use the `onPage` callback to process or accumulate items from each page.

### Basic Usage

```typescript
// Iterate all pages, collect items via onPage callback
const allProducts: Product[] = [];

const { pagination } = await sdk.admin.products.list({ per_page: 100 }).loop({
  onPage: ({ data }) => {
...
<Note>
  ...
  `.loop()`, the request works exactly as before—returning only the first page.
</Note>
...
const { data: allOrders } = await sdk.admin.orders
  .list({ status: 'processing', per_page: 100 })
  .loop({
    onPage: ({ data, pagination }) => {
      currentPage += 1;
      const percent = Math.round(
        (currentPage / (pagination.totalPages || 1)) * 100
      );
      console.log(
        `Progress: ${percent}% (${data?.length} orders, total: ${pagination.total})`
      );
    },
  });
```
...
try {
  const { data } = await sdk.admin.products.list({ per_page: 100 }).loop({
    signal: controller.signal,
    onPage: ({ pagination }) =>
      console.log(`Fetched batch, total items: ${pagination.total}`),
  });
...
When aborted, the extension returns the last fetched page's data rather than throwing an error.
...
async function fetchAllProducts() {
  ...
  const { data } = await sdk.admin.products.list().loop({
    ...
    onPage: ({ pagination }) => {
      updateProgressBar(pagination.totalPages || 0);
    },
  });
...
```typescript
const { data } = await sdk.admin.customers.list({ per_page: 100 }).loop({
...
```typescript
const { data } = await sdk.admin.orders.list({ per_page: 50 }).loop({
...
```typescript
const { data, error } = await sdk.admin.products.list().loop({
  stopOnError: false,
...
await sdk.admin.products.list({ per_page: 100 }).loop({
  onPage: async ({ data }) => {
    if (data) {
...
const { data: categories } = await sdk.store.products.categories
  .list({ per_page: 100 })
  .loop();

// Fetch all products with a specific attribute
const { data: products } = await sdk.store.products
  .list({ attribute: 'pa_color', attribute_term: 'red', per_page: 100 })
  .loop({
    onPage: ({ pagination }) => {
      console.log(`Loading products... total: ${pagination.total}`);
    },
  });
```
...
const request = sdk.admin.products.list({ status: 'publish' });
