...
The EventBus is accessible via `Typewoo.events`:
...
```typescript
Typewoo.events.once('auth:login:success', () => {
...
Typewoo.events.on('cart:updated', handleCartUpdate);

// Later: remove handler
Typewoo.events.off('cart:updated', handleCartUpdate);
```
...
```typescript
const unsubscribe = Typewoo.events.onAny((event, payload) => {
...
```typescript
const cartEvents = Typewoo.events.scope('cart:');
...
Typewoo.events.on('batch:loading', (isLoading) => {
  loadingStates.batch = isLoading;
  updateUI();
});

Typewoo.events.on('cart:extensions:loading', (isLoading) => {
...
Typewoo.events.on('cart:token:changed', async (token) => {
...
Typewoo.events.on('nonce:changed', async (nonce) => {
  await fetch('/api/session', {
    method: 'POST',
...
async function addToCartAndWait(productId: number) {
  ...
  const addPromise = Typewoo.store.cart.add({ id: productId });

  // Wait for the cart update event
  const updatedCart = await Typewoo.events.waitFor('cart:updated', undefined, 5000);
...
```typescript
import { Typewoo, SdkEvent } from '@typewoo/sdk';
...
Typewoo.events.on('invalid:event', () => {}); // Error!

// TypeScript error: wrong payload type
Typewoo.events.on('cart:loading', (cart) => {
