...
```typescript
const { data, error } = await sdk.store.cart.add({ id: 123 });
...
```typescript
const { data, error } = await sdk.store.cart.applyCoupon('INVALID');
...
Use `RequestOptions` callbacks for per-request error and retry handling:
...
const { data } = await sdk.store.products.list(
  { per_page: 10 },
  {
    ...
    onRetry: (attempt, error, context) => {
      console.log(
        `Retry attempt ${attempt} for ${context.url}:`,
        error.message
      );
    },
  }
...
The callbacks receive context about the request:
...
### Using Global Request Callback

The simplest way to handle errors globally is with the `request.onError` callback:
...
<Note>
  Global `request.onError` fires for every failed request, after any per-request
  `onError` callback. Both can be used simultaneously.
</Note>
...
sdk.events.on('auth:token:refresh:error', (error) => {
...
### Per-Request Retry Handling

Use the `onRetry` callback in request options to handle retry events:
...
const { data } = await sdk.store.products.list(
  ...
  {
    onRetry: (attempt, error, context) => {
      console.log(`Retry ${attempt} for ${context.url}:`, error.message);
    },
  }
);
```

The `onRetry` callback receives:

- `attempt`: The retry attempt number (1-based)
- `error`: The `ApiError` that triggered the retry
- `context`: The `RequestContext` with URL, method, and payload info

<Note>
  `onRetry` is also available as a global callback via `request.onRetry` in the
  SDK configuration. The per-request callback fires first, then the global one.
</Note>
...
```typescript
import { createTypewoo, httpClient } from '@typewoo/sdk';
...
```typescript
const { error } = await sdk.store.cart.add({ id: 123 });
