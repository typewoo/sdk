...
For iterating all pages automatically, use the [`.loop()` extension](/extensions#loop-extension):

```typescript
// Iterate all products with progress tracking
const allProducts: Product[] = [];

await sdk.store.products.list({ category: 'clothing', per_page: 100 }).loop({
