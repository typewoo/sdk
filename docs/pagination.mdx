...
```typescript
async function loadAllProducts(params?: ProductRequest): Promise<ProductResponse[]> {
...
```typescript
function generatePageNumbers(current: number, total: number, maxVisible = 7): (number | null)[] {
  if (total <= maxVisible) {
...
import { Typewoo, ProductResponse, ProductRequest, Pagination } from '@typewoo/sdk';
