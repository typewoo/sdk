...
```typescript
const { data, pagination, error } = await sdk.store.products.list({
  page: 1,
  per_page: 12,
});
...
For fetching all pages automatically, use the [`.loop()` extension](/extensions#loop-extension):

```typescript
// Fetch all products with progress tracking
const { data: allProducts } = await sdk.store.products
  .list({ category: 'clothing', per_page: 100 })
  .loop({
    onPage: ({ pagination }) => {
      console.log(
        `Fetched batch, total: ${pagination.total}/${pagination.totalPages} pages`
      );
    },
  });
...
<Info>
  The `.loop()` extension handles pagination automatically, supports
  cancellation via `AbortController`, and includes safety measures to prevent
  infinite loops. See [Extensions](/extensions) for full documentation.
</Info>

### Generator-Based Iteration

<Note>
  For most use cases, the [`.loop()` extension](/extensions#loop-extension)
  provides a simpler way to fetch all pages. Use generators when you need
...
```typescript
// Products
const { data, pagination } = await sdk.admin.products.list({
  page: 1,
  per_page: 20,
});

// Orders
const { data, pagination } = await sdk.admin.orders.list({
  page: 1,
  per_page: 50,
  status: 'processing',
});
...
const { data } = await sdk.store.products.list({ per_page: 12 });

// Avoid: very large pages (slow, memory intensive)
const { data } = await sdk.store.products.list({ per_page: 1000 });
```
