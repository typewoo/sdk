...
| `onError`    | `(error: ApiError, context: RequestContext) => void`     | Called on request failure                          |
| `onRetry`    | `(attempt: number, error: ApiError, context) => void`    | Called on each retry attempt                       |
