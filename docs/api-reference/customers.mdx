...
const customerService = Typewoo.admin.customers;
```
...
```typescript
const { data: customers, pagination, error } = await Typewoo.admin.customers.list({
  per_page: 20,
  page: 1,
...
```typescript
const { data: customer, error } = await Typewoo.admin.customers.getById(123);
...
```typescript
const { data: customer, error } = await Typewoo.admin.customers.create({
...
```typescript
const { data: customer, error } = await Typewoo.admin.customers.update(123, {
  first_name: 'Jane',
  last_name: 'Doe',
  billing: {
    phone: '+1555123456',
...
```typescript
// Move to trash
const { data, error } = await Typewoo.admin.customers.delete(123);

// Permanently delete
const { data, error } = await Typewoo.admin.customers.delete(123, true);

// Delete and reassign orders to another customer
const { data, error } = await Typewoo.admin.customers.delete(123, true, 456);
```
...
```typescript
const { data: downloads, error } = await Typewoo.admin.customers.downloads(123);

downloads?.forEach(download => {
  console.log(`${download.product_name}: ${download.downloads_remaining} remaining`);
});
```
...
async function findCustomerByEmail(email: string) {
  const { data } = await Typewoo.admin.customers.list({
    email,
    per_page: 1,
  });
...
async function updateCustomerAddress(
  ...
  const { data, error } = await Typewoo.admin.customers.update(customerId, {
    [type]: address,
  });
...
```typescript
const { data, error } = await Typewoo.admin.customers.create(customerData);
...
```typescript
// List coupons
const { data } = await Typewoo.admin.coupons.list();

// Create coupon
const { data } = await Typewoo.admin.coupons.create({
  code: 'SUMMER2024',
...
```typescript
// List reviews
const { data } = await Typewoo.admin.productReviews.list({
  product: [123],
});

// Create review
const { data } = await Typewoo.admin.productReviews.create({
  product_id: 123,
...
const { data } = await Typewoo.admin.settings.list();

// Get specific group
const { data } = await Typewoo.admin.settings.getGroup('general');

// Update setting
const { data } = await Typewoo.admin.settings.updateOption(
  'general',
...
```typescript
// Get sales report
const { data } = await Typewoo.admin.reports.getSales({
  date_min: '2024-01-01',
  date_max: '2024-12-31',
});

// Top sellers
const { data } = await Typewoo.admin.reports.getTopSellers({
  period: 'month',
});
```
