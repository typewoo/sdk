...
```typescript
const { data: item, error } = await Typewoo.store.cartItems.single('abc123def456');
```
...
```typescript
const { data: item, error } = await Typewoo.store.cartItems.update('abc123def456', 3);
```
...
Typewoo.events.on('cart:updated', (cart) => {
  console.log(`Cart: ${cart?.items_count} items, ${formatPrice(cart?.totals.total_price)}`);
});
...
class MiniCart {
  ...
  render() {
    ...
    console.log(`${items_count} items - ${totals.currency_symbol}${totals.total_price}`);
  }
}
```
