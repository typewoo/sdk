...
async function processCheckout(
  ...
  const { data: updatedCheckout, error: updateError } =
    await Typewoo.store.checkout.update(
      {
...
async function payForOrder(
  ...
  const { data: result, error: paymentError } =
    await Typewoo.store.checkoutOrder.order(orderId, {
      key: orderKey,
      payment_method: 'stripe',
...
```typescript
const { data, error } = await Typewoo.store.checkout.processOrderAndPayment(
  params
);
