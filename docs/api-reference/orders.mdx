...
const checkoutService = sdk.store.checkout;
const checkoutOrderService = sdk.store.checkoutOrder;
const orderService = sdk.store.orders;
```
...
```typescript
const { data: checkout, error } = await sdk.store.checkout.get();
...
```typescript
const { data: checkout, error } = await sdk.store.checkout.update(
  {
    billing_address: {
      first_name: 'John',
      last_name: 'Doe',
...
```typescript
const { data: result, error } = await sdk.store.checkout.processOrderAndPayment(
  {
    billing_address: {
      first_name: 'John',
      last_name: 'Doe',
...
```typescript
const { data: result, error } = await sdk.store.checkoutOrder.order(
  12345, // Order ID
  {
    key: 'wc_order_xxx', // Order key
    payment_method: 'stripe',
    payment_data: [{ key: 'stripe_source', value: 'src_xxx' }],
  }
);
```
...
```typescript
const { data: order, error } = await sdk.store.orders.get(
...
async function processCheckout(
  ...
  const { data: updatedCheckout, error: updateError } =
    await sdk.store.checkout.update(
      {
...
async function payForOrder(
  ...
  // 2. Process payment
  const { data: result, error: paymentError } =
    await sdk.store.checkoutOrder.order(orderId, {
      key: orderKey,
      payment_method: 'stripe',
      payment_data: [{ key: 'stripe_source', value: stripeToken }],
...
```typescript
// List orders
const { data: orders, pagination } = await sdk.admin.orders.list({
  status: 'processing',
  per_page: 20,
});

// Get single order
const { data: order } = await sdk.admin.orders.getById(12345);

// Update order status
const { data: updated } = await sdk.admin.orders.update(12345, {
  status: 'completed',
});

// Create order
const { data: newOrder } = await sdk.admin.orders.create({
  payment_method: 'bacs',
...
// Delete order
await sdk.admin.orders.delete(12345, true);

// Create refund
const { data: refund } = await sdk.admin.refunds.create(12345, {
  amount: '10.00',
...
```typescript
const { data, error } = await sdk.store.checkout.processOrderAndPayment(params);
...
if (error) {
  switch (error.code) {
    ...
    case 'woocommerce_rest_checkout_missing_required_field':
      ...
      showError(
        `Missing field: ${Object.keys(error.data.params || {}).join(', ')}`
      );
      break;
