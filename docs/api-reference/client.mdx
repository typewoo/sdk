...
The Typewoo SDK is created through the `createTypewoo()` factory, which returns a typed client instance. This page documents the main client interface.
...
### `createTypewoo(config: SdkConfig): TypewooClient`

Create an SDK instance with configuration options. Returns the client instance synchronously.
...
Access WooCommerce Analytics API services.

```typescript
const revenue = await sdk.analytics.revenue.getStats({
  after: '2026-01-01',
  before: '2026-01-31',
  interval: 'day',
});

const { data: orders, pagination } = await sdk.analytics.orders.list({
  per_page: 25,
  after: '2026-01-01',
  before: '2026-01-31',
});
...
| `orders`       | `AnalyticsOrdersService`       | Order analytics rows and stats           |
...
| `variations`   | `AnalyticsVariationsService`   | Variation-level performance analytics    |
| `customers`    | `AnalyticsCustomersService`    | Customer analytics and spend reporting   |
| `downloads`    | `AnalyticsDownloadsService`    | Download activity analytics              |
| `stock`        | `AnalyticsStockService`        | Inventory status and stock analytics     |
| `performance`  | `AnalyticsPerformanceService`  | Performance indicators and allowed stats |
| `leaderboards` | `AnalyticsLeaderboardsService` | Leaderboard reports                      |

**Notes:**

- Analytics uses WooCommerce Analytics endpoints under `/wp-json/wc-analytics/...`
- List-style analytics methods return an awaitable paginated request, so `await sdk.analytics.orders.list()` works directly
- If analytics routes are protected, the SDK can reuse admin credentials when `admin.useAuthInterceptor` is enabled
...
A promise that resolves once the initial authentication state has been read
from storage. `createTypewoo()` returns synchronously, so await `ready` before
relying on `state.authenticated` at startup.
...
**Type:** `Promise<void>` — never rejects; a failing storage provider leaves
the instance unauthenticated.
...
don't cover. Requests go through the instance's own Axios client, so they use
its `baseUrl`, interceptors, retry settings and request hooks.

```typescript
const { data, error } = await sdk.http.get<Post[]>('/wp-json/wp/v2/posts');
await sdk.http.post('/wp-json/custom/v1/items', { name: 'Item' });

// The instance's Axios client, e.g. for your own interceptors
sdk.http.client.interceptors.request.use((config) => config);
```

**Type:** `TypewooHttp` — `get`, `post`, `put`, `delete`, `head`, `client`

### `doGet`, `doPost`, `doPut`, `doDelete`, `doHead`

Free helpers with the same behaviour. Without a context argument they use the
**first** instance created, so prefer `sdk.http` (or the `endpoints` factory)
when your app creates more than one instance.

### `httpClient` (deprecated)

A proxy to the first instance's Axios client. Use `sdk.http.client` instead.
