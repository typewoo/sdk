...
```typescript
const { data, error } = await Typewoo.store.batch.execute({
  requests: [
    {
      method: 'POST',
...
const { data } = await Typewoo.store.batch.execute({
...
```typescript
const { data, error } = await Typewoo.store.cartExtensions.update({
...
interface RequestOptions<T = unknown> {
  ...
  onResponse?: (response: T, context: RequestContext<T>) => void | Promise<void>;
  onError?: (error: ApiError, context: RequestContext<T>) => void | Promise<void>;
...
const cookieStorage = (key: string, req: Request, res: Response): StorageProvider => ({
  get: async () => {
...
import { Typewoo, ProductRequest } from '@typewoo/sdk';
...
export class StoreService {
  ...
  getProducts(params?: ProductRequest): Observable<ProductResponse[]> {
    return from(
      Typewoo.store.products.list(params).then(({ data, error }) => {
        if (error) throw new Error(error.message);
        return data!;
      })
    );
  ...
  addToCart(productId: number): Observable<CartResponse> {
    return from(
      Typewoo.store.cart.add({ id: productId }).then(({ data, error }) => {
        if (error) throw new Error(error.message);
        return data!;
      })
    );
...
export function useCart() {
  ...
  onMounted(() => {
    unsubscribe = Typewoo.events.on('cart:updated', (newCart) => {
      cart.value = newCart;
    });
  ...
  const addToCart = async (productId: number) => {
    const { error } = await Typewoo.store.cart.add({ id: productId });
    if (error) throw new Error(error.message);
  };
...
async function preloadProducts() {
  await Typewoo.store.products.list({ per_page: 12 });
}
...
const [productsResult, cartResult, categoriesResult] = await Promise.all([
  Typewoo.store.products.list({ per_page: 12 }),
  Typewoo.store.cart.get(),
  Typewoo.store.categories.list(),
]);
```
...
async function getCart() {
  return cache.dedupe('cart', () => Typewoo.store.cart.get());
}
```
...
```typescript
// Log all events
Typewoo.events.onAny((event, payload) => {
...
console.log('SDK State:', {
  authenticated: Typewoo.state.authenticated,
  cartToken: Typewoo.state.cartToken,
  nonce: Typewoo.state.nonce,
});
```
