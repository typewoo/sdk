...
For SSR, you may need request-scoped SDK instances. Because `createTypewoo()`
returns an independent instance on each call, you can create one per request:
...
function createRequestSdk(req: Request, res: Response) {
  return createTypewoo({
    baseUrl: 'https://your-store.com',
    // Use a request-scoped storage provider (see below)
  });
}

// For SSR, also consider:
