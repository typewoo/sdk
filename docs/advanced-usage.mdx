...
<Note>
  ...
  option. See [Configuration — Global Lifecycle
...
Each instance owns its own Axios client, exposed as `sdk.http.client`.
Interceptors you add there only apply to that instance.

```typescript
const sdk = createTypewoo({ baseUrl: 'https://your-store.com' });

// Add custom interceptor
sdk.http.client.interceptors.request.use((config) => {
  config.headers['X-Request-ID'] = generateRequestId();
  return config;
});

// Add response interceptor
sdk.http.client.interceptors.response.use(
  (response) => {
...
sdk.http.client.interceptors.response.use((response) => {
...
In server-side environments, `localStorage` and `sessionStorage` are unavailable. The SDK falls back to memory storage automatically, but emits a console warning. To avoid the warning, either set storage explicitly to `'memory'` or set `suppressStorageWarnings: true`:

```typescript
// Explicit memory storage — recommended for SSR (no warning, no fallback)
const sdk = createTypewoo({
  baseUrl: 'https://your-store.com',
  auth: {
    accessToken: {
...
// Or suppress the warning when using the automatic fallback
const sdk = createTypewoo({
  baseUrl: 'https://your-store.com',
...
For SSR, create one SDK instance per incoming request. Each instance has its
own Axios client, interceptors, config and state, so tokens, nonces and cart
tokens never leak between requests, and nothing accumulates on a shared client:

```typescript
// Create a fresh, request-scoped instance for each incoming request
function createRequestSdk(req: Request, res: Response) {
  return createTypewoo({
    baseUrl: 'https://your-store.com',
    auth: {
      // Request-scoped storage (see "Custom Storage Provider" below)
      accessToken: { storage: cookieStorage('access_token', req, res) },
      refreshToken: { storage: cookieStorage('refresh_token', req, res) },
    },
...
<Note>
  Use request-scoped (or `'memory'`) storage for every instance. A storage
  provider shared between requests would share its tokens too.
</Note>
