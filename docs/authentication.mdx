...
Use the `sdk.auth.token()` method to authenticate users:
...
```typescript
const { data } = await sdk.auth.token({
...
2. `sdk.state.authenticated` is set to `false`
...
```typescript
console.log('Is authenticated:', sdk.state.authenticated);
```
...
const { data } = await sdk.auth.oneTimeToken({
  redirect: '/my-account',
});

// Build the auto-login URL
const autoLoginUrl = await sdk.auth.getAutoLoginUrl(
  data.token,
  '/my-account',
...
<Tip>
  If your auto-login endpoint is on a different domain than your Store API, set
  `auth.autoLoginUrl` in the SDK configuration. When provided,
  `getAutoLoginUrl()` uses it instead of `baseUrl`. See the [Configuration
  docs](/configuration#auto-login-url) for details.
</Tip>
...
const { data: products } = await sdk.admin.products.list();
```
...
sdk.events.on('auth:token:refresh:start', () => {
  console.log('Token refresh started');
});

sdk.events.on('auth:token:refresh:success', () => {
...
sdk.events.on('auth:token:revoke:start', () => {
  console.log('Logout started');
});

sdk.events.on('auth:token:revoke:success', () => {
