...
The SDK is organized with four main service groups:
...
│   ├── coupons
│   └── ... (many more)
├── analytics (WooCommerce Analytics API)
│   ├── revenue
│   ├── orders
│   ├── products
│   ├── categories
...
The recommended way to create an SDK instance is using `createTypewoo()`:
...
You can create multiple SDK instances for different stores:

```typescript
const storeA = createTypewoo({ baseUrl: 'https://store-a.com' });
const storeB = createTypewoo({ baseUrl: 'https://store-b.com' });

// Use independently
const productsA = await storeA.store.products.list();
const productsB = await storeB.store.products.list();
```

<Note>
  The legacy `Typewoo.init()` singleton was removed in v4. Each call to
  `createTypewoo()` returns an independent instance.
</Note>
...
Analytics services interact with WooCommerce Analytics endpoints:

| Service                        | Property                 | Endpoint Base                                          |
| ------------------------------ | ------------------------ | ------------------------------------------------------ |
| `AnalyticsRevenueService`      | `analytics.revenue`      | `/wp-json/wc-analytics/reports/revenue/stats`          |
| `AnalyticsOrdersService`       | `analytics.orders`       | `/wp-json/wc-analytics/reports/orders`                 |
| `AnalyticsProductsService`     | `analytics.products`     | `/wp-json/wc-analytics/reports/products`               |
| `AnalyticsCategoriesService`   | `analytics.categories`   | `/wp-json/wc-analytics/reports/categories`             |
| `AnalyticsCouponsService`      | `analytics.coupons`      | `/wp-json/wc-analytics/reports/coupons`                |
| `AnalyticsTaxesService`        | `analytics.taxes`        | `/wp-json/wc-analytics/reports/taxes`                  |
| `AnalyticsVariationsService`   | `analytics.variations`   | `/wp-json/wc-analytics/reports/variations`             |
| `AnalyticsCustomersService`    | `analytics.customers`    | `/wp-json/wc-analytics/reports/customers`              |
| `AnalyticsDownloadsService`    | `analytics.downloads`    | `/wp-json/wc-analytics/reports/downloads`              |
| `AnalyticsStockService`        | `analytics.stock`        | `/wp-json/wc-analytics/reports/stock`                  |
| `AnalyticsPerformanceService`  | `analytics.performance`  | `/wp-json/wc-analytics/reports/performance-indicators` |
| `AnalyticsLeaderboardsService` | `analytics.leaderboards` | `/wp-json/wc-analytics/leaderboards`                   |

If the store protects analytics endpoints, these requests can reuse the same admin credentials configured for `sdk.admin`.

## SDK State

The SDK maintains runtime state accessible via `sdk.state`:
...
console.log(sdk.state.cartToken);

// After authentication
console.log(sdk.state.authenticated);
```
...
// sdk.events.emit('cart:updated', cartData);
...
Each SDK instance creates its own Axios client, configured from `baseUrl` and
`axiosConfig`, with its own interceptors. It's available as `sdk.http.client`,
and `sdk.http` exposes request helpers bound to that client:
...
The free request helpers below behave the same way, but target the first
instance created unless you pass a context:
